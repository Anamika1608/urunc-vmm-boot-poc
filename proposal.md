...
My open source work has been moving toward this project for a while. I have not been a long-time urunc contributor yet, so I do not want to pretend otherwise. What I do have is a trail of work in systems-adjacent code where correctness, test coverage, process behavior, packaging, and performance mattered.

In DataHaskell/DataFrame, I worked on real library behavior instead of only documentation. PR #187 fixed join behavior when key columns were missing. That work forced me to reason about how a library should fail, how users discover data-shape problems, and how tests should pin edge cases. PR #190 added documentation and tests around Parquet `safeColumns`, and PR #195 adds Brotli page decompression support. That last one is especially relevant to my systems interests because it sits near a file format boundary: bytes on disk, compression codec behavior, and how a high-level user experiences a low-level storage choice.

I have also contributed to the P4 ecosystem. My merged work in `p4lang/behavioral-model` and `p4lang/ptf`, plus release and packaging work in `p4lang/tutorials`, gave me a lot of practice reading large projects before touching them. Network tooling punishes shallow guesses. I had to understand how pieces fit together before proposing a change, which is exactly the habit I need for urunc. A low-level runtime is not the place for a flashy patch that breaks lifecycle guarantees.

On the runtime and infrastructure side, I have contributed work around sandbox startup and observability. In `zenact-ai/sontaran`, PR #495 stopped OpenTelemetry collector readiness from blocking sandbox bootstrap. PR #500 added sandbox worker warm-up mode. PR #507 improved structured logging. In `java-device-executor`, PR #32 added tracing and logging. Those are not VMM patches, but the theme is close to issue #112: find unnecessary blocking work, move it out of the startup path without lying about readiness, and leave enough visibility that the next engineer can debug failures.

My day-to-day tools are Go, Rust, C, Linux, Git, and the usual runtime debugging stack. I am comfortable reading unfamiliar code, building a local model of the lifecycle, and then writing small tests before making a larger change. For this proposal, I read the urunc lifecycle code, the hypervisor implementations for Firecracker, QEMU, HVT, and SPT, the network package, the IPC path, the Firecracker API, QMP references, and related approaches from Kata, gVisor, and Cloud Hypervisor. That research made the project feel less like an abstract mentorship idea and more like a concrete patch series I can build with the maintainers.
